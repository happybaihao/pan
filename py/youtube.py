    ...
    "自然": [
        {
            ...
            "value": [
                {"n": "全部", "v": ""},
                {"n": "风景", "v": "大自然 风景 nature scenery"},
                {"n": "动物世界", "v": "动物世界 wildlife documentary"},
                {"n": "海洋", "v": "海洋 自然 ocean nature"},
                {"n": "森林", "v": "森林 自然 forest nature"},
                {"n": "鸟类", "v": "鸟类 自然 birds nature"},
                {"n": "地球", "v": "地球 自然 earth nature"},
                {"n": "国家地理", "v": "National Geographic nature wildlife"},
                {"n": "BBC Earth", "v": "BBC Earth nature"}
            ]
        }
    ],
    "HDR": [
        {
            ...
            "value": [
                {"n": "全部", "v": ""},
                {"n": "风景", "v": "HDR 风景 scenery"},
                {"n": "自然", "v": "HDR nature"},
                {"n": "动物", "v": "HDR wildlife animals"},
                {"n": "城市", "v": "HDR city"},
                {"n": "演示片", "v": "HDR demo video"},
                {"n": "放松", "v": "HDR relax"}
            ]
        }
    ]
...
CATEGORY_QUERY = {
    'YouTube 直播 24小時': 'YouTube 直播 24小時',
    'YouTube 新聞 Live': 'YouTube 新聞 Live',
    '劇集': '电视剧 剧集 drama',
    '電影': '电影 movie',
...
class YouTubeLite:
    ...
    def __init__(self, session, headers=None, config=None):
        ...
        self.sig_plan_cache = {}
        self.nsig_cache = {}
        self.extract_cache_ttl = int(self.config.get('extract_cache_ttl') or 14400)
    ...
    # ---------- NewPipe 风格：VISIONOS 优先 + ANDROID_VR 快速返回 ----------
    def _call_player_api(self, video_id, api_key, context, referer, visitor_data=None, sts=None):
        # NewPipe 风格：VISIONOS 使用 googleapis 端点，其他使用 www.youtube.com
        visionos_ctx = {'client': {'clientName': 'VISIONOS', 'clientVersion': '1.02', 'deviceMake': 'Apple', 'deviceModel': 'RealityDevice14,1', 'userAgent': 'com.google.visionos.youtube/1.02(RealityDevice14,1; U; CPU visionOS 25_6_0 like Mac OS X;)', 'osName': 'visionOS', 'osVersion': '25.6.0', 'hl': 'en', 'gl': 'US'}}
        clients = [
        ...
        try:
            ...
            headers = {
                ...
                'User-Agent': client.get('userAgent') or '',
            }
            if visitor_data:
            ...
            debug_log('player api VISIONOS', {'status': status, 'has_streaming': has_streaming, 'formats': len(formats), 'adaptive': len(adaptive), 'direct_any': len(direct_any), 'direct_video': len(direct_video)})
            if has_streaming:
                data['_client_name'] = client_name
                data['_client_ua'] = client.get('userAgent')
                results.append(data)
                if direct_video:
                    debug_log('VISIONOS fast return', {'direct_video': len(direct_video)})
                    return results
                if fallback is None:
                    fallback = data
        except Exception as e:
            debug_log('VISIONOS client error', repr(e))
        ...
        for ctx in clients:
            ...
            try:
                ...
                if has_streaming:
                    ...
                    results.append(data)
                    # ANDROID_VR 有 direct_video 即快速返回
    ...
    def _decrypt_nsig(self, media_url, player_url):
        """解密 YouTube 视频 URL 中的 n 参数（节流参数），防止播放被限速断流"""
        try:
            # 先从查询参数中获取 n 值
            n_value = None
            query_match = re.search(r'[?&]n=([^&]+)', media_url)
            if query_match:
                n_value = unquote(query_match.group(1))
            # 再尝试从路径中获取 n 值
            path_match = re.search(r'/n/([^/?]+)', media_url)
            if not n_value and path_match:
                n_value = unquote(path_match.group(1))
            if not n_value:
                return media_url
            # 获取 n-transform 计划并解密
            n_plan = self._get_n_transform_plan(player_url)
            if n_plan:
                decrypted_n = self._decrypt_n_by_plan(n_value, n_plan)
                if decrypted_n and decrypted_n != n_value:
                    fixed = media_url
                    # 替换查询参数中的 n 值（保持原始编码方式）
                    if query_match:
                        # 使用正则替换，保持其他参数不变
                        encoded_n = quote(decrypted_n, safe='')
                        fixed = re.sub(
                            r'([?&]n=)[^&]+',
                            r'\g<1>' + encoded_n,
                            fixed,
                    ...
                    # 替换路径中的 n 值
                    if path_match:
                        encoded_n_path = quote(decrypted_n, safe='')
                        fixed = re.sub(
                            r'(/n/)[^/?]+',
                            r'\g<1>' + encoded_n_path,
                            fixed,
                            count=1
                        )
                    debug_log('n decrypted', {'original_len': len(n_value), 'decrypted_len': len(decrypted_n), 'changed': fixed != media_url})
                    return fixed
            # 如果无法解密，至少同步路径和查询中的 n 值
            if query_match and path_match:
                query_n = unquote(query_match.group(1))
                path_n = unquote(path_match.group(1))
                if query_n != path_n:
                    # 以查询参数为准，同步路径
                    fixed = re.sub(
                        r'(/n/)[^/?]+',
                        r'\g<1>' + query_match.group(1),
                        media_url,
                        count=1
                    )
                    debug_log('n path synced (fallback)', {'old_len': len(path_n), 'new_len': len(query_n)})
                    return fixed
            debug_log('n present (no decrypt)', {'n_len': len(n_value), 'has_path_n': bool(path_match)})
            return media_url
        except Exception as e:
            debug_log('n decrypt error', repr(e))
            return media_url

    def _get_n_transform_plan(self, player_url):
        """获取 n 参数变换操作计划（带缓存）"""
        cache_key = player_url or ''
        ...
        code = self._get_player_code(player_url)
        if not code:
            self.nsig_cache[cache_key] = None
            return None
        plan = self._extract_n_function_plan(code)
        self.nsig_cache[cache_key] = plan
        debug_log('n plan extracted', {'player_len': len(code), 'has_plan': bool(plan), 'plan_len': len(plan) if plan else 0})
        return plan
    ...
    def _extract_helper_object(self, code, name):
        ...
        if not body:
            return {}
        result = {}
        # 匹配方法定义：methodName:function(params){body}
        for method, method_body in re.findall(r'([a-zA-Z0-9_$]+)\s*:\s*function\([a-z,]+\)\s*\{(.*?)\}', body, re.S):
            if '.reverse(' in method_body:
                result[method] = 'reverse'
            elif '.splice(' in method_body:
                result[method] = 'splice'
            elif '.slice(' in method_body:
                result[method] = 'slice'
            elif 'a[0]' in method_body and 'length' in method_body and '%' in method_body:
                result[method] = 'swap'
        return result

    def _extract_n_function_plan(self, code):
        """提取 n 参数变换函数的操作计划（NewPipe 风格，支持 helper 对象）"""
        if not code:
            return None
        name = None
        # NewPipe 风格的 n 函数名检测模式（按优先级排序）
        patterns = [
            r'\.get\("n"\)\)&&\(a=([a-zA-Z0-9_$]+)(?:\[(\d+)\])?\(a\)',
            r'\.get\("n"\)\)&&\(b=([a-zA-Z0-9_$]+)(?:\[(\d+)\])?\(b\)',
            r'\.get\("n"\)\)&&\(b=([a-zA-Z0-9_$]+)\(b\)',
            r'[a-zA-Z0-9_$]=String\.fromCharCode\(110\),[a-zA-Z0-9_$]=[a-zA-Z0-9_$]\.get\([a-zA-Z0-9_$]\)\)&&\([a-zA-Z0-9_$]=([a-zA-Z0-9_$]+)(?:\[(\d+)\])?\([a-zA-Z0-9_$]\)',
            r'([a-zA-Z0-9_$]+)=function\(a\)\{a=a\.split\(""\);',
            r'([a-zA-Z0-9_$]+)=function\(a\)\{var b=a\.split\(""\)',
            r'function\s+([a-zA-Z0-9_$]+)\(a\)\{a=a\.split\(""\)',
        ...
        for pattern in patterns:
            ...
            if m:
                name = m.group(1)
                if m.lastindex and m.lastindex >= 2 and m.group(2) and m.group(2).isdigit():
                    array_index = int(m.group(2))
                break
        if not name:
            debug_log('n func name not found')
            return None
        # 处理数组索引的情况（NewPipe 风格：函数名来自数组）
        if array_index is not None:
            arr_match = re.search(r'(?:var\s+|let\s+|const\s+)?' + re.escape(name) + r'\s*=\s*\[([^\]]+)\]', code)
            if arr_match:
                names = [n.strip().strip('"').strip("'") for n in arr_match.group(1).split(',')]
                if array_index < len(names):
                    name = names[array_index]
                    debug_log('n func from array', {'index': array_index, 'name': name})
                else:
                    debug_log('n func array index out of range', {'index': array_index, 'len': len(names)})
                    return None
        body = self._extract_js_function_body(code, name)
        debug_log('n function body', {'name': name, 'body_len': len(body)})
        if not body:
            return None
        # 检测数组变量名（可能是 a, b, c 等）
        arr_var = 'a'
        split_match = re.search(r'(?:var\s+)?([a-zA-Z0-9_$]+)\s*=\s*a\.split\(', body)
        if split_match:
            arr_var = split_match.group(1)
        else:
            direct_match = re.search(r'([a-zA-Z0-9_$]+)\s*=\s*\1\.split\(', body)
            if direct_match:
                arr_var = direct_match.group(1)
        debug_log('n array var', {'var': arr_var})
        # 提取 helper 对象（与签名解密相同的方式，但使用动态变量名）
        helper_pattern = r'([a-zA-Z0-9_$]+)\.[a-zA-Z0-9_$]+\(' + re.escape(arr_var) + r',\d+\)'
        helper = self._search(helper_pattern, body)
        helper_map = self._extract_helper_object(code, helper) if helper else {}
        debug_log('n helper', {'helper': helper, 'methods': list(helper_map.keys())})
        # 构建操作计划
        plan = []
        for part in body.split(';'):
            part = part.strip()
            if not part:
                continue
            # reverse()
            if re.search(re.escape(arr_var) + r'\.reverse\(\)', part):
                plan.append(('reverse', 0))
                continue
            # .slice(N)
            m = re.search(re.escape(arr_var) + r'\.slice\((\d+)\)', part)
            if m:
                plan.append(('slice', int(m.group(1))))
                continue
            # .splice(0,N)
            m = re.search(re.escape(arr_var) + r'\.splice\(0,\s*(\d+)\)', part)
            if m:
                plan.append(('splice', int(m.group(1))))
                continue
            # helper.method(arr, N)
            if helper:
                m = re.search(re.escape(helper) + r'\.([a-zA-Z0-9_$]+)\(' + re.escape(arr_var) + r',\s*(\d+)\)', part)
                if m:
                    op = helper_map.get(m.group(1))
                    if op:
                        plan.append((op, int(m.group(2))))
                        continue
            # a = a.join("") 等末尾语句，忽略
        debug_log('n plan', {'plan': plan, 'len': len(plan)})
        return plan or None

    def _decrypt_n_by_plan(self, n_value, plan):
        """使用操作计划解密 n 参数"""
        if not plan:
            return n_value
        arr = list(n_value)
...
class YouTubeLiveLite:
    ...
    def __init__(self, session, headers=None, config=None):
        ...
        self.config = config or {}
        self.cache = {}
        self.cache_ttl = int(self.config.get('live_cache_ttl') or 300)
    ...
    def extract_live(self, url_or_id):
        ...
        # 判断是否为正在直播：isLiveContent=True 且无 VOD 格式（adaptiveFormats/formats）
        # 已结束的直播录像和首播虽然有 isLiveContent=True，但有 VOD 格式，应走 DASH 路径
        has_vod_formats = bool(streaming.get('adaptiveFormats') or streaming.get('formats'))
        is_live = bool(details.get('isLiveContent')) and not has_vod_formats
...
class Spider(Spider):
    def getName(self):
        # 不含"YouTube"/"youtube"/"油管"关键字
        # 否则APK内置app.py的apply_system_proxy会清空session.proxies
        return 'YT视频直播（优化版）'
    ...
    def init(self, extend):
        ...
        # ----- 代理配置：直接使用星落TV内置Mihomo代理 -----
        # 星落TV 5.8.8 (包名 xinghe.tv) 内置 Mihomo(Clash Meta内核)
        # com.fongmi.android.tv.proxy.b.k() 硬编码返回端口 18890
        # Mihomo进程随APP启动, mixed-port=18890, 无需检测直接使用
        # 如果ext中配置了proxy则优先使用ext配置, 否则默认127.0.0.1:18890
        proxy = self.extendDict.get('proxy', '127.0.0.1:18890')
        if isinstance(proxy, str):
            ...
            if proxies:
                ...
                self.proxy_str = (proxies.get('https') or proxies.get('http') or '127.0.0.1:18890').replace('http://', '').replace('https://', '')
            else:
                self.session.proxies = {'http': 'http://127.0.0.1:18890', 'https': 'http://127.0.0.1:18890'}
                self.proxy_str = '127.0.0.1:18890'
        else:
            self.session.proxies = {'http': 'http://127.0.0.1:18890', 'https': 'http://127.0.0.1:18890'}
            self.proxy_str = '127.0.0.1:18890'
        debug_log('代理配置', {'proxy': self.proxy_str})
        # ---------------------------------------------------------

        # 检测星落TV内置代理服务器端口 (9978-9999, 对应 APK T3.c)
        self.app_proxy_port = self._detect_app_proxy_port()
    ...
    def _ensure_proxy(self):
        """安全机制：确保session代理未被清除
        APK内置app.py的apply_system_proxy在init()之后运行，
        可能会清空session.proxies（当名称含YouTube/油管且ext无proxy时）。
        此方法在每个关键网络请求前调用，重新设置Mihomo代理。"""
        mihomo_proxy = 'http://127.0.0.1:18890'
        if not getattr(self, 'session', None):
            return
        current = self.session.proxies or {}
        if not current.get('http') or '18890' not in str(current.get('http', '')):
            self.session.proxies = {'http': mihomo_proxy, 'https': mihomo_proxy}
        if not getattr(self, 'proxy_str', '') or '18890' not in getattr(self, 'proxy_str', ''):
            self.proxy_str = '127.0.0.1:18890'

    def _proxy_base(self):
        """返回内置代理服务器基础URL, 对应 APK com.github.catvod.Proxy.getUrl(true)
        5.8.8: T3.c.e(true) 返回 http://127.0.0.1:{port}"""
        return f'http://127.0.0.1:{self.app_proxy_port}/proxy'

    def _get_stream_proxy(self):
        """返回流代理字符串, 供 localProxy 的 application/x-codex-stream-url 响应使用
        对应 APK com.fongmi.chaquo.Spider.createProxy() + isLocalMihomo() 检查:
        - 127.0.0.1:18890 → 使用 SOCKS 类型 (Mihomo mixed-port)
        - 其他 → 使用 HTTP 类型"""
        if getattr(self, 'proxy_str', '') and '18890' in self.proxy_str:
            return 'http://127.0.0.1:18890'
        elif getattr(self, 'proxy_str', ''):
            return f'http://{self.proxy_str}'
        return ''

    def _detect_app_proxy_port(self):
        """检测星落TV内置代理服务器端口
        5.8.8: 对应 T3.c.i(): 扫描 9978-9999 找到可用端口, Proxy.set(port)"""
        import socket
    ...
    def _check_mihomo_proxy(self):
        """检测星落TV内置Mihomo代理是否运行
        5.8.8: 对应 com.fongmi.android.tv.proxy.b:
        - b.k() 返回固定端口 18890
        - b.d() socket连接 127.0.0.1:18890, 100ms超时
        - b.s() 等待启动完成, 最多50次×100ms=5秒
        - b.l(str) 生成代理URI: http://127.0.0.1:18890#节点名
        - 配置: mixed-port:18890, external-controller:127.0.0.1:18891, allow-lan:false
        - 代理组: XYS_PROXY (select类型), 规则: CN域名直连+MATCH走代理"""
        import socket
        # 第一步: socket端口检测 (对应 b.d() 方法, 100ms超时)
        try:
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(0.1)
            sock.connect(('127.0.0.1', 18890))
            sock.close()
        except Exception:
            return False
        # 第二步: HTTP CONNECT验证 (验证Mihomo确实在代理, 而非端口被占用)
        for attempt in range(3):
            try:
                ...
                if '200' in first_line:
                    debug_log('Mihomo CONNECT测试成功', {'attempt': attempt, 'response': first_line})
                    return True
            except Exception:
                try:
                    sock.close()
                    ...
                    pass
                time.sleep(1)
        debug_log('Mihomo端口可达但CONNECT测试失败')
        return False

    def _auto_detect_proxy(self):
        """自动检测可用代理，Mihomo端口(18890)优先，然后常见代理端口"""
        proxy_list = [
            "http://127.0.0.1:18890",  # 星落TV Mihomo (Clash Meta内核, 内置)
            "http://127.0.0.1:2080",  # 油管通用代理
            "http://127.0.0.1:7890",   # Clash 默认
            "http://127.0.0.1:10809", # V2Ray 默认
            "http://127.0.0.1:20172", # 其他
            "http://127.0.0.1:7891",  # Clash 备用
            "http://127.0.0.1:10808", # V2Ray 备用
            "http://127.0.0.1:1087",  # Shadowsocks
            "http://127.0.0.1:3128",  # Squid
            "http://127.0.0.1:1080",  # SOCKS 通用
            "http://127.0.0.1:8080",  # HTTP 通用
            "http://127.0.0.1:9090"   # 其他
        ]
        for p in proxy_list:
            try:
    ...
    def detailContent(self, did):
            ...
            if hls_url:
                ...
                if variants:
                    for v in variants:
                        ...
                        self.setCache(cache_key, {'url': v['url'], 'expires': time.time() + 1800})
        ...
        vod = {
            'vod_id': video_id,
            'vod_name': title,
            'vod_pic': f'{self._proxy_base()}?do=py&type=image&vid={video_id}',
            'vod_remarks': status,
        ...
        try:
            ...
            if audio:
                ...
                return {
                    'parse': 0,
                    'jx': 0,
                    'url': f'{self._proxy_base()}?do=py&type=mpd&vid={video_id}&quality={target_height}_{hdr_type}',
    ...
    def _parse_renderer(self, renderer, is_live=False):
        try:
            ...
            return {
                ...
                'vod_pic': f'{self._proxy_base()}?do=py&type=image&vid={vid}',
                'vod_remarks': remarks
            }
        except Exception:
            return None
    ...
    def _play_live(self, video_id):
        try:
            ...
            # 如果不是正在直播（如已结束的直播录像/首播），回退到 VOD 路径
            if not data.get('is_live'):
                debug_log('_play_live not live, fallback to VOD', {'video_id': video_id, 'has_hls': bool(data.get('hls_url'))})
                return self._play_video(video_id, 'best')
        ...
        try:
            ...
            if playable:
                ...
                if audio:
                    ...
                    return {'parse': 0, 'jx': 0, 'url': f'{self._proxy_base()}?do=py&type=mpd&vid={video_id}&quality={quality}', 'format': 'application/dash+xml'}
    ...
    HLS_TTL = {'master': 6 * 3600, 'playlist': 6 * 3600, 'media': 600, 'media_retry': 600}
    ...
    def _cache_hls_url(self, target_url, video_id='', kind='media'):
        ...
        return f'{self._proxy_base()}?do=py&type=hls&key={quote(key)}'
    ...
    def _proxy_single(self, params):
        ...
        # 5.8.8: 使用流代理机制
        stream_response = {'url': target_url, 'headers': headers}
        proxy_str = self._get_stream_proxy()
        if proxy_str:
            stream_response['proxy'] = proxy_str
        stream_response['content_type'] = data.get('content_type', 'video/mp4')
        return [200, 'application/x-codex-stream-url', stream_response]

    def _refresh_video_cache(self, vid, quality):
        """缓存过期时自动重新获取视频信息，防止播放中断流"""
        if not vid:
            return False
        try:
            debug_log('auto refresh video cache', {'vid': vid, 'quality': quality})
            # 判断 quality 格式，调用对应的播放方法
            if quality and '_' in quality:
                parts = quality.rsplit('_', 1)
                if len(parts) == 2 and parts[1] in ('sdr', 'hdr') and parts[0].isdigit():
                    self._play_video_by_height_and_type(vid, int(parts[0]), parts[1])
                    return True
            # 其他格式走通用播放路径
            self._play_video(vid, quality or 'best')
            return True
        except Exception as e:
            debug_log('auto refresh cache error', {'vid': vid, 'quality': quality, 'error': repr(e)})
            return False

    def _proxy_mpd(self, params):
        vid = params.get('vid')
        quality = params.get('quality') or '1080p'
        cache_key = f'yt_{vid}_{quality}' if vid else None
        data = self.getCache(cache_key) if cache_key else None
        if not data and vid:
            # 缓存过期，自动刷新
            if self._refresh_video_cache(vid, quality):
        ...
        media_base = f'{self._proxy_base()}?do=py&type=media&vid={vid}&quality={quality}'
...
'''
    ...
    def _proxy_media(self, params):
        ...
        if range_header:
            headers['Range'] = range_header
        # 5.8.8: 使用 application/x-codex-stream-url 让APK通过Mihomo代理流媒体
        # 对应 com.fongmi.chaquo.Spider.getRemoteStream(): 读取url/proxy/headers/content_type
        stream_response = {'url': target_url, 'headers': headers}
        proxy_str = self._get_stream_proxy()
        if proxy_str:
            stream_response['proxy'] = proxy_str
        content_type = (media_item or {}).get('mimeType', 'application/octet-stream').split(';')[0]
    ...
    def _proxy_hls(self, params):
        ...
        try:
            ...
            # 5.8.8: 非m3u8媒体分段使用流代理机制
            response.close()
            stream_response = {'url': target_url, 'headers': headers}
            proxy_str = self._get_stream_proxy()
            if proxy_str:
                stream_response['proxy'] = proxy_str
            stream_response['content_type'] = content_type or 'application/octet-stream'
            return [200, 'application/x-codex-stream-url', stream_response]
        except Exception as e:
