...
# @Time    : 2026/8/7
"""
全能直播聚合插件 - 点播版 v12_final（纯配置驱动）增强版
- 所有名称规范化规则（繁简映射、频道别名、过滤词等）均从 JSON 配置读取
- Python 代码不包含任何硬编码映射数据
- 兼容影视TV环境，支持 config_file 外部配置
- 增强：每个接口分类加载完成后立即更新缓存，切换分类即可看到已就绪数据
- 增强：首次焦点阻塞等待，不返回空数据；超时或无效数据显示提示项
"""
import re
import sys
import os
...
class Spider(BaseSpider):
    ...
    def __init__(self):
        ...
        # ========== 新增状态属性（增强加载） ==========
        self._source_loaded = {}           # {source_name: bool} 标记每个接口源是否加载完成
        self._global_ready = False         # 所有源是否加载完成
        self._first_source_name = None     # 第一个接口分类名称
        self._focus_count = {}             # {tid: int} 记录每个分类被焦点的次数（本版暂未使用，保留）
        # ============================================
    ...
    def init(self, extend):
        ...
        self.logger.log("=" * 50 + " 启动 VOD v9_final（纯配置驱动）增强版 " + "=" * 50)
        ...
        # 初始化源加载状态（第一个源已同步加载，标记为 True）
        for cfg in self._source_configs:
            if cfg['name'] == self._first_source_name:
                self._source_loaded[cfg['name']] = True
            else:
        ...
        if first_cfg:
            ...
            self._load_one_source(first_cfg, is_first=True)  # 标记为第一个，不触发全局完成检查
        ...
        for cfg in self._source_configs[1:]:
            ...
            with self._channels_lock:
                if name in self._source_groups:
                    continue
            self._load_one_source(cfg, is_first=False)  # 非第一个
            time.sleep(0.2)
    ...
    # ========== 动态构建静态分类内容（从 _channels 实时筛选） ==========
    def _dynamic_static_category_content(self, tid, pg):
        """从全局 _channels 中筛选属于该静态分类的频道（利用 _category_map）"""
        channel_names = self._category_map.get(tid, [])
        if not channel_names:
            return {"list": [], "page": 1, "pagecount": 1, "limit": 30, "total": 0}
        # 从 _channels 中筛选出规范名在 channel_names 中的频道
        with self._channels_lock:
            matched_channels = []
            for ch in self._channels:
        ...
        for ch in matched_channels:
            ...
            # 去重（按 url）
            existing_urls = {s['url'] for s in merged[norm]['sources']}
    ...
    def detailContent(self, ids):
        ...
        with self._cache_lock:
            ...
            if cat_type == 'interface':
                ...
                original_director = " | ".join(play_from_list)  # 线路来源
                original_actor = f"共{len(play_from_list)}个频道"  # 线路数量

                vod = {
                    "vod_id": vid,
                    "vod_name": item,
                    "vod_pic": self.default_vod_pic,
                    "vod_director": cat_or_interface,  # 分类名称
                    "vod_actor": original_director,    # 原线路来源
