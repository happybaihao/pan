...
# 远程域名配置地址
DOMAIN_TXT_URL = "http://xk996.xkgzs.xyz/1/123.txt"
# 域名缓存有效期 3600秒(1小时)，防止频繁请求txt
DOMAIN_CACHE_EXPIRE = 3600
...
class Spider(Spider):
    def __init__(self):
        self.host = ""
        self.did = ""
        self.domain_cache_time = 0  # 域名缓存时间戳

    def _get_remote_host(self):
        """从远程txt读取主域名，带缓存"""
        now = int(time.time())
        # 缓存未过期，直接使用已有域名
        if self.host and (now - self.domain_cache_time) < DOMAIN_CACHE_EXPIRE:
            return self.host
        try:
            # 请求远程配置文件
            resp = requests.get(DOMAIN_TXT_URL, timeout=8, headers={"User-Agent": "okhttp/3.14.9"})
            resp.raise_for_status()
            # 去除首尾空格、换行符
            domain = resp.text.strip()
            if domain:
                self.host = domain
                self.domain_cache_time = now
                return self.host
        except Exception as e:
            print("读取远程域名失败：", e)
        # 读取失败兜底原域名
        self.host = "http://154.29.148.82:6966"
        return self.host
    ...
    def getdata(self, path, data=None):
        try:
            ...
            resp = self.post(full_url, headers=self.header(), data=data, timeout=10).json()['data']
            data1 = self.aes(resp)
